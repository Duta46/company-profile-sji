...
class PartnerController extends Controller
{
            ...
            return DataTables::of($partner)
                ...
                ->addColumn('actions', function ($item) {
                    ...
                    <div class="menu-item px-3">
                            <a href="' . route('Partner.edit', $item->id) . '" class="menu-link px-3">
                                Edit Data
                            </a>
                        </div>
                        <div class="menu-item px-3">
                ...
                ->editColumn('image', function ($item) {
                    if ($item->image) {
                        $imagePath = "storage/partner/{$item->image}";
                        $imageUrl = asset($imagePath);
                        return  '<img src="' . $imageUrl . '" width="30%" height="30%">';
                    } else {
                        return '-';
                    }
                })
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $imageQuestion = $request->file('image');
            $originalImageQuestion = Str::random(10) . $imageQuestion->getClientOriginalName();
            $imageQuestion->storeAs('public/partner', $originalImageQuestion);
            $data['image'] = $originalImageQuestion;
        } else {
            $data['image'] = null;
        }
        ...
        return redirect()->route('Partner.index')->with('success', 'Berhasil Tambah Partner');
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        return redirect()->route('Partner.index')->with('success', 'Berhasil Ubah Partner');
    }
